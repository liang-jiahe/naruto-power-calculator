...
describe('首页快捷导航', () => {
  ...
  it.each(['power', 'upgrade', 'magic', 'materials'] as const)('首页卡片可以进入 %s，回首页后恢复根地址', (workspace) => {
    ...
    const workspaceUrl = new URL(routeHref(workspace), window.location.origin)
    expect(window.location.pathname).toBe(workspaceUrl.pathname)
    expect(window.location.hash).toBe('')
  ...
  it('工具页侧边栏仍可进入全部战力分项，定位后滚动并聚焦目标区', async () => {
    ...
    for (const [id, label] of POWER_NAV_ITEMS) {
      ...
      const sectionUrl = new URL(routeHref('power', id), window.location.origin)
      expect(window.location.pathname).toBe(sectionUrl.pathname)
      expect(window.location.hash).toBe(sectionUrl.hash)
    }
...
describe('地址恢复与历史导航', () => {
  ...
  it('浏览器后退和前进会同步可见板块与地址', async () => {
    ...
    expect(window.location.pathname).toBe(new URL(routeHref('upgrade'), window.location.origin).pathname)
    act(() => window.history.back())
