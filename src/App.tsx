          ...
          <div className="content workspace-view" hidden={activeWorkspace !== 'power'}>
            ...
            <section id="collection" className="bonus-panel">
              <img
                ...
                aria-hidden="true"
              />
              <div><h2>忍者收集加成</h2><p>修改一次，全页面所有计算立即联动。</p></div>
            ...
            <SectionCard id="level" title="等级战力" description="按 20–170 级收益表精确查表，并叠加全局收集倍率。" value={result.sections.level.total} formula={formulas.level} tone="blue">
            ...
            <SectionCard id="soul" title="忍魂" description="忍传感悟吃收集加成；羁绊升级按基础系数直接换算。" value={result.sections.soul.total} formula={formulas.soul}>
            ...
            <SectionCard id="talent" title="天赋" description="普通天赋与修罗天赋分别计算，再汇入天赋总战力。" value={result.sections.talent.total} formula={formulas.talent} tone="purple">
            ...
            {([
              ['equipment', '装备', '装备三维属性吃全局收集倍率。', 'blue'],
              ['magatama', '勾玉', '勾玉生命、攻击、防御统一计算。', 'green'],
            ] as const).map(([key, title, description, tone]) => (
              <SectionCard key={key} id={key} title={title} description={description} value={result.sections[key].total} formula={formulas[key]} tone={tone}>
            ...
            <SectionCard id="accessories" title="饰品与符文" description="饰品属性、六件符文直接战力和自动匹配的共鸣属性合并计算。" value={result.sections.accessories.total} formula={formulas.accessories} tone="green">
            ...
            <SectionCard id="artifact" title="神器" description="神器三维属性吃全局收集倍率。" value={result.sections.artifact.total} formula={formulas.artifact} tone="purple">
            ...
            <SectionCard id="summoning" title="通灵" description="修炼属性由通灵兽、等级与强化次数查表；进阶属性由总面板扣除修炼值。" value={result.sections.summoning.total} formula={formulas.summoning}>
            ...
            <section id="tools" className="section-card tone-blue">
              <header className="section-head"><div><h2>忍具系统</h2><p>穿戴、重铸、藏馆与珍品阁分项计算，避免模式混用。</p></div>
            ...
            {([
              ['scroll', '秘卷', '秘卷三维属性吃全局收集倍率。', 'green'],
              ['outfit', '装扮', '装扮属性按基础系数计算，不吃收集加成。', 'purple'],
              ['title', '称号', '称号三维属性吃全局收集倍率。', 'orange'],
              ['avatar', '头像框', '头像框三维属性吃全局收集倍率。', 'blue'],
            ] as const).map(([key, title, description, tone]) => (
              <SectionCard key={key} id={key} title={title} description={description} value={result.sections[key].total} formula={formulas[key]} tone={tone}>
                <CoreEditor value={state.simple[key]} onChange={(field, value) => updateSimple(key, field, value)} />
                <BreakdownStrip data={result.sections[key]} />
